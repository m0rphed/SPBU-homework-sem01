...
void processData(const string &filename, Record *phoneBook, int &size)
{
	...
	while (!data.eof() && size < fixedSize)
	{
		...
		if (!temp.empty())
		{
			...
			getline(data, temp);
			string::size_type sz;
			phoneBook[size].phoneNumber = stol(temp);
			++size;
		}
	}
	
	data.close();
	cout << "Успешно прочитанно " << size << " записей." << endl;
}
...
void phoneBookUserInterface()
{
	...
	const string filenameIn = "phoneBook-data.txt";
	...
	processData(filenameIn, phoneBook, size);
	...
	do
	{
		...
		switch (command)
		{
			...
			case 5:
				...
				saveData(filenameOut, phoneBook, size);
				break;
